...
- 2026-06-09 local verification and app polish commit: the mounted app now has richer group/activity iconography, a Wallet receive QR/share sheet, haptics on primary actions, clearer home action items, one-line guarded text in dense cards, a multi-step New group wizard with stablecoin selection, and wallet quick actions wired to Receive/Send/Telegram instead of inert buttons. Verification: `npm run typecheck`, `npm run verify:devnet`, `npm run verify:production`, and `npm run verify:fundwise-sync:force` pass. `verify:production` still warns until `EXPO_PUBLIC_SOLANA_RPC_ENDPOINT` is set to a paid mainnet RPC. Local physical-device QA was not rerun in this pass because `adb devices` returned no connected Android device.
- 2026-06-09 production wiring pass: aligned Seeker with the hardened FundWise production contracts. The mounted app now turns MWA wallet approval into a FundWise browser-session-compatible wallet session by calling `/api/auth/wallet/challenge` and `/api/auth/wallet/verify` with a `signMessages()` signature before protected FundWise calls. Mobile Settlement Request preview now calls `/api/mobile/settlement-requests/{requestId}/preview` with cookie/session auth instead of the removed `wallet=` query shortcut. Link parsing now preserves production `inviteToken` values from `/groups/{id}?invite=true&inviteToken=...`, raw `FWI-...` tokens, and `/join` links, and the shared invite lookup helper uses `/api/group-invites/preview` for tokenized invites. `npm run verify:production` now fails if Seeker regresses to wallet-query settlement preview, drops invite-token parsing, or stops wiring MWA signatures into the FundWise session. `npm run verify:fundwise-sync:force` now checks the sibling FundWise auth, invite, and mobile-preview source contracts, not only file presence. Verification: `npm run typecheck`, `npm run verify:devnet`, `npm run verify:production`, and `npm run verify:fundwise-sync:force` pass. `npm run verify:production:strict` still fails until `EXPO_PUBLIC_SOLANA_RPC_ENDPOINT` is set to the production paid mainnet RPC. Physical-device QA is still blocked because `adb devices` shows no connected Android device.
